    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          console.log(`[Realtime] Canal "${channelName}" ativo.`);
        } else if (status === 'CHANNEL_ERROR') {
          console.warn('[Realtime] Erro no canal — verifique se a publicação está ativa no Supabase.');
        }
      });
  ...
  const handleSaveAuth = async (newPerson) => {
    try {
      const dbPerson = {
        ...
        status: newPerson.status || 'pending',
