  ...
  if (turmas.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white rounded-3xl border border-slate-200 shadow-sm p-8 text-center">
        <ClipboardList className="text-slate-300 w-12 h-12 mb-3" />
    ...
    <div className="h-full flex flex-col bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 sm:p-6 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">Observação Diária</h2>
            <p className="text-slate-500 text-sm hidden sm:block">Registre observações pedagógicas sobre seus alunos.</p>
          </div>
        </div>
        {!showForm && (
          <button
      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 sm:p-5 space-y-3">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-slate-800 text-sm">Nova observação</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm uppercase"
                >
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Atividade escolhida</label>
                <input
                  type="text"
                  value={form.atividade}
                  onChange={e => setForm({ ...form, atividade: e.target.value })}
                  className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm uppercase placeholder:normal-case"
                  placeholder="Ex: Torre Rosa"
                />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Material utilizado</label>
                <input
                  type="text"
                  value={form.material}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Nível de concentração</label>
                <select
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Autonomia</label>
                <select
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Foco observado</label>
              <input
                type="text"
                value={form.foco}
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-xs font-semibold text-slate-700">Necessitou de orientação do guia</span>
            </label>

            <div>
              <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Observações</label>
              <textarea
        ...
        {isLoading ? (
          ...
          filteredRecords.map(r => {
            ...
            const c = r.content || {};
            return (
              <div key={r.id} className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm">
                <div className="flex justify-between items-start gap-3">
                  <div className="min-w-0">
                    <h4 className="font-bold text-slate-800">{student?.name || 'Aluno removido'}</h4>
                    <p className="text-slate-500 text-xs mt-0.5">{formatDate(r.record_date)}{student?.turma ? ` — ${student.turma}` : ''}</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {c.atividade && <span className="text-[11px] font-semibold bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg">Atividade: {c.atividade}</span>}
                  {c.material && <span className="text-[11px] font-semibold bg-slate-50 text-slate-600 px-2 py-1 rounded-lg">Material: {c.material}</span>}
                  {c.concentracao && <span className="text-[11px] font-semibold bg-slate-50 text-slate-600 px-2 py-1 rounded-lg">Concentração: {NIVEL_CONCENTRACAO.find(n => n.value === c.concentracao)?.label || c.concentracao}</span>}
                  {c.autonomia && <span className="text-[11px] font-semibold bg-slate-50 text-slate-600 px-2 py-1 rounded-lg">Autonomia: {NIVEL_AUTONOMIA.find(n => n.value === c.autonomia)?.label || c.autonomia}</span>}
                ...
                {c.foco && <p className="text-slate-600 text-sm mt-2"><strong className="font-semibold text-slate-700">Foco:</strong> {c.foco}</p>}
                {c.observacoes && <p className="text-slate-600 text-sm mt-2 whitespace-pre-wrap">{c.observacoes}</p>}
              </div>
            );
          })
        )}
