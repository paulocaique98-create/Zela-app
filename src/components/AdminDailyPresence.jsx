...
import { GraduationCap, Clock, LogOut, CheckCircle2, Users, RefreshCw } from 'lucide-react';
...
export default function AdminDailyPresence() {
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-400">
      ...
      <div className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 text-sm font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-4 py-2 rounded-xl transition disabled:opacity-50"
          >
      ...
      {/* Sub-menu de Turmas */}
      <div className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-200">
        <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl w-fit mb-6 overflow-x-auto">
          {TURMAS.map(turma => (
            <button
              key={turma}
