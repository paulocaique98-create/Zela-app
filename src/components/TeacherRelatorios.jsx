...
import { FileText, Loader2, Plus, X, Check, ArrowLeft, Send, Search } from 'lucide-react';
...
export default function TeacherRelatorios({ currentUser, currentSchool }) {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [reportsRes, studentsRes, templatesRes] = await Promise.all([
        supabase.from('reports').select('*').eq('school_id', schoolId).eq('author_id', currentUser.id).order('updated_at', { ascending: false }).limit(300),
        supabase.from('students').select('id, name, turma').eq('school_id', schoolId).in('turma', turmas).order('name', { ascending: true }),
      ...
      console.error('[TeacherRelatorios] Erro ao buscar dados:', err);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      console.error('[TeacherRelatorios] Erro ao criar relatório:', err);
  ...
  const openReport = async (reportId) => {
    ...
    try {
      ...
      console.error('[TeacherRelatorios] Erro ao abrir relatório:', err);
  ...
  const saveReport = async (extra = {}) => {
    ...
    try {
      ...
      console.error('[TeacherRelatorios] Erro ao salvar:', err);
  ...
  // ── Detalhe de um relatório (preencher seções + publicar) ──
  if (activeReport) {
    const student = studentsById.get(activeReport.student_id);
        ...
        <form onSubmit={handleCreate} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-3">
          {templates.length === 0 ? (
            <div className="bg-amber-50 border border-amber-200 text-amber-700 p-4 rounded-xl text-sm font-medium">
              Nenhum modelo de relatório disponível ainda. Peça à administração para criar um.
            </div>
          ) : (
            <>
              <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 sm:p-6 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm hidden sm:block">Relatórios de Desempenho e Evolução dos seus alunos.</p>
          </div>
        </div>
