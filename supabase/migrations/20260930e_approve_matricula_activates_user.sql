-- O link publico de Matricula (novo) cria a conta do titular com
-- status='pending' (mesmo padrao do autocadastro /cadastro) -- sem isso, a
-- familia nunca conseguiria logar mesmo depois da escola aprovar a
-- matricula, porque nada nunca ativava a conta. Ativa o responsavel junto
-- com a aprovacao (seguro tambem pra Rematricula, onde a conta ja e
-- 'active' e o UPDATE nao muda nada).
...
BEGIN
  ...
  -- 1. Enriquece o cadastro do responsável financeiro e ativa a conta (só
  -- tem efeito real pro link público de Matrícula, que nasce 'pending').
  UPDATE users SET
    status = 'active',
